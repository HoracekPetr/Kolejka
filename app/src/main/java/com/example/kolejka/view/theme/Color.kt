package com.example.kolejka.view.theme

import androidx.compose.material.lightColors
import androidx.compose.ui.graphics.Color

/*val Purple200 = Color(0xFFBB86FC)
val Purple500 = Color(0xFF6200EE)
val Purple700 = Color(0xFF3700B3)
val Teal200 = Color(0xFF03DAC5)*/

val LightBackgroundWhite = Color(0xA6F1EAEE)
val PostWhite = Color(0xFFFAF7F9)
val PostDark = Color(0xFF535353)
val DarkBackgroundGray = Color(0x73A2A2A2)
val ExtraLightGray = Color(0xFFD6D4D4)
val LightGray = Color(0xFFC4C4C4)
val DarkGray = Color(0xFF747476)
val BlackAccent = Color(0xFF1D010E)
val DarkPurple = Color(0xFF7E1A47)
val PostStripEvent = Color(0xFFCFA1AE)
val PostStripOffer = Color(0xFFEECBD5)
val MediumOpaquePurple = Color(0xBE7E1A47)
val LightOpaquePurple = Color(0x807E1A47)
val LightPurple = Color(0xFFCF4483)
val MediumGreen = Color(0xFF67992D)
val LightGreen = Color(0xFF92DB3C)
val MediumRed = Color(0xFFB91919)
val LightRed = Color(0xFFFF4040)

private val LightColors = lightColors()